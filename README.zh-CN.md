...
Codex Switcher 是一个 macOS 菜单栏应用，用于在同一台 Mac 上管理多个 Codex / ChatGPT 账号。它可以通过 OpenAI OAuth 登录添加账号，展示小时和周额度的剩余用量，刷新用量，在授权失效时重新验证账号，并通过更新标准 Codex CLI 授权文件来切换当前账号。
...
- 展示小时额度和周额度的剩余用量
- 支持刷新单个账号或全部账号
- 刷新返回 HTTP 401 时，可在账号卡片中点击“验证”重新登录并更新 token
- 可选择在唤起面板时自动刷新
- 支持按小时和周剩余额度阈值自动切换账号
- 支持英文和简体中文语言设置，默认跟随受支持的系统语言
...
如果刷新用量时返回 HTTP 401，账号卡片会显示红色“验证”按钮。点击后会为该账号重新执行 OAuth 登录流程，并在返回的 ChatGPT account id 与当前卡片一致后覆盖本地 token。

`1455` 端口的服务由 Codex Switcher 自己启动，具体实现在 `OAuthLoginService` 中。它只在添加账号和验证账号流程中临时监听本机 `localhost` 回调，收到回调或取消后会关闭。
